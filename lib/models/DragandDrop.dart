...
class DragDropGameScreenState extends State<DragDropGameScreen>
  ...
  List<PuzzlePiece> get currentPuzzlePieces {
    var pieces = List<PuzzlePiece>.from(puzzleLevels[currentLevel - 1]);
    if (currentLevel >= 2) {
      pieces.shuffle();
    }
    return pieces;
  ...
  Widget _buildLevel4DropBoxes() {
    return GridView.count(
      ...
      crossAxisSpacing: 20,
      children: currentPuzzlePieces.map((piece) {
        return _buildMovingBox(piece);
      }).toList(),
    );
  }

  Widget _buildMovingBox(PuzzlePiece piece) {
    final controller = AnimationController(
      duration: Duration(seconds: 8 + currentPuzzlePieces.indexOf(piece)),
      vsync: this,
    );
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final scale = 15.0;
        final x = scale * sin(animation.value);
        final y = scale * sin(animation.value) * cos(animation.value);
